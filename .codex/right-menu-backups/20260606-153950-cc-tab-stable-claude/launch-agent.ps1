...
function Get-AgentCommand {
    ...
    switch ($Agent) {
        ...
        "claude-code" { return [pscustomobject]@{
            Name = "claude"
            Candidates = @(
        ...
        "gemini" { return [pscustomobject]@{
            Name = "gemini"
            Candidates = @(
                (Join-Path $env:APPDATA "npm\gemini.cmd"),
                (Join-Path $env:APPDATA "npm\gemini.ps1")
            )
        } }
        default { return $null }
    }
...
function Resolve-AgentCommand {
    ...
    $spec = Get-AgentCommand $Agent
    if (-not $spec) { return $null }

    $commandName = $spec.Name
    $candidatePaths = @($spec.Candidates)
